    ...
    <div className="h-full flex flex-col bg-sidebar border-r border-sidebar-border">
      ...
      <div className="px-3 py-3 border-b border-sidebar-border">
        <div className="flex items-center gap-1 mb-2">
          <span className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider flex-1">Workspaces</span>
      ...
      {activeWorkspace && (
        <>
          ...
          {/* Tasks */}
          <button onClick={onOpenTasks} className="mx-3 mt-2 flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-sidebar-accent transition-colors text-sm text-muted-foreground hover:text-foreground">
              ...
              <AnimatePresence>
                {showChannels && (
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden space-y-0.5">
                    ...
                    {channels.length === 0 && <p className="text-xs text-muted-foreground px-2 py-1">No channels</p>}
                  </motion.div>
                )}
              </AnimatePresence>
            </div>
                ...
                {showMembers && (
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden space-y-0.5">
                    {members.map((m) => {
                      ...
                      return (
                        <div key={m.user_id} className="flex items-center gap-2 px-2 py-1 rounded-lg">
          ...
          <div className="px-3 py-2 border-t border-sidebar-border relative">
            <button onClick={() => setShowStatusPicker(v => !v)}
              ...
              <span className="text-xs text-muted-foreground flex-1 text-left">{myStatus.label}</span>
              <Settings className="h-3 w-3 text-muted-foreground" />
            </button>
            <AnimatePresence>
              {showStatusPicker && (
                <motion.div initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 4 }}
                  className="absolute bottom-12 left-3 right-3 bg-card border border-border rounded-xl shadow-xl overflow-hidden z-20">
                  {DEV_STATUSES.map((s) => (
                    <button key={s.id} onClick={() => { onSetDevStatus(s.id); setShowStatusPicker(false); }}
